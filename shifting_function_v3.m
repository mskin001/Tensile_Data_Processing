...
% times = raw_times;
% comp = log10(raw_comp);
ref_time = log10(exp_time{1}(begin_point(1):end))';
ref_comp = log10(comp{1}(begin_point(1):end))';
non_lin = [NaN;NaN];
best_fit_exten = 10000;
% figure(), hold on, grid on
for k = 1:length(temps) -1
% find overlab region
    next_time = log10(exp_time{k+1}(begin_point(k+1):end))';
    next_comp = log10(comp{k+1}(begin_point(k+1):end))';
%     hold on
...
%     plot(next_time, next_comp)
    ...
    while isnan(log_shift_factor) || isinf(log_shift_factor)
        ...
        try
          log_shift_factor = (A - b) / (L(end,2) - U(1,2)) - shift_modifier(k);
        catch
          log_shift_factor = NaN;
        end
    ...
    non_lin = [non_lin(1,:), shifted_time(1:25)'; non_lin(2,:), next_comp(1:25)'];
...
figure()
hold on
plot(ref_time,mc, 'k^', 'MarkerFaceColor', 'k', 'LineWidth', 1.5)
plot(non_lin(1,:),10.^non_lin(2,:), 'b^', 'LineWidth', 1.5)
marker = ['o+^d'];
for k = 1:length(temps)
    hold on
    plot(log10(exp_time{k}), comp{k}, marker(k),'Linewidth', 1.5)
end
plot(fit_time, 10.^fit_comp, '-', 'Color', [0.6350 0.0780 0.1840], 'Linewidth', 1.5)
grid on
set(gca, 'Fontsize', 12)
xlabel('log_{10}(t) [sec]')
ylabel('Compliance [1/Pa]')
legend({'Shifted Curves', 'Non-linear Creep', '30^oC', '45^oC', '60^oC', 'Master Curve'},...
    'Location', 'southeast')
